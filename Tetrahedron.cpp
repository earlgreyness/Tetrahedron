...
void Tetrahedron::resizeGL(int width, int heigth)
{
    glViewport(0, 0, width, heigth);
    glMatrixMode(GL_PROJECTION);
    glLoadIdentity();
    GLfloat x = GLfloat(width) / heigth;
...
void Tetrahedron::draw()
{
    ...
    for (int i = 0; i < 4; ++i) { // 4 грани у тетраэдра
        //glLoadName(i);
        glBegin(GL_TRIANGLES);
        qglColor(faceColors[i]);
        for (int j = 0; j < 3; ++j) { // 3 вершины у каждого треугольника - грани тетраэдра
